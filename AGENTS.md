...
- `catalogId` 는 도료 하나를 가리키는 고유 id 다 (`tamiya.enamel.x-1`).
  **한 번 정하면 바꾸지 않는다** — 사용자가 등록한 도료가 `paints.catalog_id` 로
  이 값을 들고 있고, 같은 도료를 두 번 등록하는 걸 이걸로 막는다.
  카탈로그를 다시 만들 때도 기존 id 는 그대로 옮겨야 한다.
  품번만으로는 부족하다 (타미야 X-1 은 아크릴·에나멜·락카에 다 있다).
...
- 브랜드는 저장할 때 `ensureBrand()` 로 만들어 붙이므로 `brands` 테이블에 미리 없어도 된다.
  `brandLine` 은 **같은 이름으로 팔리는 다른 제품 라인**에만 쓴다
  (GSI Creos 의 Mr.COLOR / 수성 호비컬러 / 아크리시온).
  락카·에나멜·아크릴처럼 **도료의 종류는 브랜드가 아니라 `type` 에 넣는다.**
  (타미야는 한 브랜드이고, 아크릴/에나멜/락카/스프레이는 `type` 으로 갈린다.)
  같은 품번이 종류마다 있어도(타미야 X-1) 카탈로그 `id` 는 `line` 으로 갈려 안 겹친다.
- **브랜드 목록은 카탈로그가 있는 브랜드만 보여 준다.** 목록은 손으로 적지 않고
  ...
  브랜드는 지운다 (붙어 있던 도료는 "브랜드 없음"이 된다).
